...
- AI 行情分析：自动查询提及币种的 Gate 标记价、24h 成交额、资金费率、全市场未平仓量（OI）以及 15m/4h K 线摘要；30 分钟内的追问会沿用同一用户上一轮币种。
...
- 持仓频道：记录多单/空单、数量、入场价和杠杆，按 Gate 标记价查询实时盈亏，并在卡片顶部醒目汇总总未实现盈亏、总保证金和整体收益率。
...
- AI 行情分析使用 Gate 公开的 ticker、合约统计和 K 线接口；OI 或某一周期 K 线临时失败时会保留其他已成功取得的数据。
...
- 持仓整体收益率按“总未实现盈亏 ÷ 总保证金”计算；若某笔无法取价，会明确显示成功计价笔数，且不会把部分合计冒充全部收益。
